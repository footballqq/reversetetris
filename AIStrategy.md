...
- 决策：加权评分（见下文公式）
- 失误：有随机扰动（会偶尔选到次优落点）
- 速度：更慢（约 800ms/步，视觉更“人类”）
- 特殊块概率：10%
- 天花板下降：每放置 10 块下降 1 格

### NORMAL
- 决策：加权评分（无随机）
- 速度：中等（约 500ms/步）
- 特殊块概率：8%
- 天花板下降：每放置 15 块下降 1 格

### HARD
- 决策：加权评分（无随机；`lookahead` 仍为占位未实现）
- 速度：更快（约 300ms/步）
- 特殊块概率：6%
- 天花板下降：每放置 20 块下降 1 格

### GOD
- 决策：词典序硬优先级（先不触顶/不留洞/不造洞…，再谈消行与高度；详见下文）
- 速度：最快（约 200ms/步）
- 特殊块概率：4%
- 天花板下降：每放置 25 块下降 1 格
- 特点：非常“保守不留洞”，因此更难被喂死
